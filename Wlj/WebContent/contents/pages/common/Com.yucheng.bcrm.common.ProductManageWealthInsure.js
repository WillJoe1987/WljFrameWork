 ...
 * 2015-05-27
 */
Com.yucheng.crm.common.ProductManageWealthInsure = Ext.extend(Ext.form.TwinTriggerField, {
	
	initComponent : function(){
		Com.yucheng.crm.common.ProductManageWealthInsure.superclass.initComponent.call(this);
	},

	onRender : function(ct, position){
		Com.yucheng.crm.common.ProductManageWealthInsure.superclass.onRender.call(this, ct, position);
		if(this.hiddenName){
			var ownerForm = this;
	...
	onTrigger2Click : function(){
		...
		var searchFunction = function(){
			...
			ProductManageWealthInsureInfoStore.removeAll();
			ProductManageWealthInsureInfoStore.baseParams = {
					'condition':Ext.util.JSON.encode(parameters)
			};
				ProductManageWealthInsureInfoStore.load({
					params:{
						'idStr':idStr,
		...
		var feeTypeStore = new Ext.data.Store({
			...
			proxy : new Ext.data.HttpProxy({
						url : basepath + '/lookup.json?name=AFEE_TYPE'
					}),
		...
		//保险类别
		var invTypeStore = new Ext.data.Store( {
			sortInfo : {
				field : 'key',
			...
			autoLoad : true,
			proxy : new Ext.data.HttpProxy( {
				url : basepath + '/lookup.json?name=INV_TYPE'   //待修改
			}),
			reader : new Ext.data.JsonReader( {
				root : 'JSON'
			}, [ 'key', 'value' ])
		...
		var productLeftTreeForShow = new Com.yucheng.bcrm.TreePanel({//左侧显示的树
			...
			clickFn:function(node){//单击事件，当单击树节点时触发并且获得这个节点的CATL_CODE
				...
				if(node.attributes.id!= ''){
					...
					idStr = "";
					ProductManageWealthInsureInfoStore.load({//重新加载产品列表
						params:{
						   'idStr':idStr,
						    fitCust:_this.fitCust,
							limit:parseInt(spagesize_combo.getValue()),
		...
		var tbar = new Ext.Toolbar({				
			items : [{ 
				text : '选定',
				handler : function() {
					var checkedNodes = ProductManageWealthInsureGrid.getSelectionModel().selections.items;
					...
					ProductManageWealthInsureWindow.hide();
		...
		var productInfoColumns = new Ext.grid.ColumnModel([rownum,sm,
		    {header:'保险名称',dataIndex:'productName',id:'productName',sortable : true,width:110},
	        {header :'产品编号',dataIndex:'productId',id:"productId",sortable : true,width:80},
	        {header:'公司名称',dataIndex:'COM_NAME',sortable : true,width:110},
	        {header:'保险类别',dataIndex:'INV_TYPE_ORA',sortable : true,width:80},
	        {header:'产品风险',dataIndex:'RISK_LEVEL_ORA',id:'productName',sortable : true,width:80},
	        {header:'',dataIndex:'RISK_LEVEL',hidden:true},
	        {header:'',dataIndex:'INV_TYPE',hidden:true},
	        {header:'',dataIndex:'FEE_TYPE',hidden:true},
	        {header:'',dataIndex:'DIV_TYPE',hidden:true},
	        {header:'预期报酬率(%)',dataIndex:'EXP_YIELD_RATE',sortable : true,width:100},
	        {header:'缴纳方式',dataIndex:'FEE_TYPE_ORA',id:'productName',sortable : true,width:80},
	        {header:'缴费期限',dataIndex:'FEE_TERM',id:'productName',sortable : true,width:100},
	        {header:'起始保费',dataIndex:'START_AMT',sortable:true,width:100},
	        {header:'分红方式',dataIndex:'DIV_TYPE_ORA',sortable:true,width:80}
	        ]);
		...
		var ProductManageWealthInsureInfoStore = new Ext.data.Store({
			proxy:new Ext.data.HttpProxy({
				url:basepath+'/comProductTreeWealthInsure-action.json',
				method:'GET'
			}),
		...
		var bbar = new Ext.PagingToolbar({	// 分页工具栏
			...
			store : ProductManageWealthInsureInfoStore,
			displayInfo : true,
		...
		var ProductManageWealthInsureGrid =  new Ext.grid.GridPanel({//产品列表数据grid
			id:'ProductManageWealthInsureGrid',
			frame:true,
			autoScroll : true,
			tbar:tbar,
			bbar:bbar,
		...
		var ProductManageWealthInsureWindow=new Ext.Window({
			title : '产品选择',
			closable : true,
			plain : true,
			resizable : true,
...
//			       productLeftTreeForShow,
				...
				items:[productSearchPanel,{
					region:'center',
					layout:'fit',
					items:[ProductManageWealthInsureGrid]
				}]				
			}]
		}); 
		ProductManageWealthInsureWindow.show();
		searchFunction();
		return;
	}
});
Ext.reg('productChoose',Com.yucheng.crm.common.ProductManageWealthInsure);
