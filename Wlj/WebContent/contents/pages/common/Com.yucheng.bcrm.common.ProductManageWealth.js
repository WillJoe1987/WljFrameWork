 ...
 * 2015-05-22
 */
Com.yucheng.crm.common.ProductManageWealth = Ext.extend(Ext.form.TwinTriggerField, {
	
	initComponent : function(){
		Com.yucheng.crm.common.ProductManageWealth.superclass.initComponent.call(this);
	},

	onRender : function(ct, position){
		Com.yucheng.crm.common.ProductManageWealth.superclass.onRender.call(this, ct, position);
		if(this.hiddenName){
			var ownerForm = this;
	...
	onTrigger2Click : function(){
		...
		var searchFunction = function(){
			...
			ProductManageWealthInfoStore.removeAll();
			ProductManageWealthInfoStore.baseParams = {
					'condition':Ext.util.JSON.encode(parameters)
			};
				ProductManageWealthInfoStore.load({
					params:{
						'idStr':idStr,
		...
		// 是否可销售
		var valIdYnStore = new Ext.data.Store({
			restful : true,
			autoLoad : true,
			proxy : new Ext.data.HttpProxy({
						url : basepath + '/lookup.json?name=VAL_ID_YN&empty=0'
					}),
			reader : new Ext.data.JsonReader({
						root : 'JSON'
					}, ['key', 'value'])
		...
		var prodTypeStore = new Ext.data.Store({
			...
			proxy : new Ext.data.HttpProxy({
						url : basepath + '/lookup.json?name=PROD_TYPE'
					}),
			reader : new Ext.data.JsonReader({
						root : 'JSON'
					}, ['key', 'value'])
		...
		var assetTypeStore = new Ext.data.Store( {
			...
			autoLoad : true,
			proxy : new Ext.data.HttpProxy( {
				url : basepath + '/lookup.json?name=ASSET_TYPE'   //待修改
			}),
			reader : new Ext.data.JsonReader( {
				root : 'JSON'
			}, [ 'key', 'value' ])
		...
		var productLeftTreeForShow = new Com.yucheng.bcrm.TreePanel({//左侧显示的树
			...
			clickFn:function(node){//单击事件，当单击树节点时触发并且获得这个节点的CATL_CODE
				...
				if(node.attributes.id!= ''){
					...
					idStr = "";
					ProductManageWealthInfoStore.load({//重新加载产品列表
						params:{
						   'idStr':idStr,
						    fitCust:_this.fitCust,
							limit:parseInt(spagesize_combo.getValue()),
		...
		var tbar = new Ext.Toolbar({				
			items : [{ 
				text : '选定',
				handler : function() {
					var checkedNodes = ProductManageWealthGrid.getSelectionModel().selections.items;
					...
					ProductManageWealthWindow.hide();
		...
		var productInfoColumns = new Ext.grid.ColumnModel([rownum,sm,
	        ...
	        {header:'产品名称',dataIndex:'productName',id:'productName',sortable : true,width:130},
//	        {header :'产品状态',dataIndex:'PROD_STATE_ORA',id:"productId",sortable : true,width:80},
	        {header:'风险等级',dataIndex:'RISK_LEVEL_ORA',id:'productName',sortable : true,width:100},
	        {header:'',dataIndex:'RISK_LEVEL',hidden:true},
	        {header:'',dataIndex:'PROD_STATE',hidden:true},
	        {header:'',dataIndex:'CATL_CODE',hidden:true},
	        {header:'产品类别',dataIndex:'CATL_NAME',sortable : true,width:100},
	        {header:'产品币种',dataIndex:'CUR_CODE_ORA',sortable:true,width:100},
	        {header:'是否可销售',dataIndex:'VAL_ID_YN_ORA',sortable:true,width:100},
	        {header:'到期日',dataIndex:'PROD_END_DATE',sortable:true,width:100}
...
//	        }}
		...
		var ProductManageWealthInfoStore = new Ext.data.Store({
			proxy:new Ext.data.HttpProxy({
				url:basepath+'/comProductTreeWealth-action.json',
				method:'GET'
			}),
		...
		var bbar = new Ext.PagingToolbar({	// 分页工具栏
			...
			store : ProductManageWealthInfoStore,
			displayInfo : true,
		...
		var ProductManageWealthGrid =  new Ext.grid.GridPanel({//产品列表数据grid
			id:'ProductManageWealthGrid',
			frame:true,
			autoScroll : true,
			tbar:tbar,
			bbar:bbar,
		...
		var ProductManageWealthWindow=new Ext.Window({
			title : '产品选选择',
			closable : true,
			plain : true,
			resizable : true,
			...
			items:[productLeftTreeForShow,{
				...
				items:[productSearchPanel,{
					region:'center',
					layout:'fit',
					items:[ProductManageWealthGrid]
				}]				
			}]
		}); 
		ProductManageWealthWindow.show();
		searchFunction();
		return;
	}
});
Ext.reg('productChoose',Com.yucheng.crm.common.ProductManageWealth);
